...
class EVAnsatz : public dolfin::Expression
{
public:
  // Constructor
            ...
            Eigen::Ref<const Eigen::VectorXd> x) const
  {
    ...
    if (dh <= 0.0) dolfin::error("Strange stuff: dh <= 0.0");
    ...
    for (std::size_t i = 0; i < n; i++)
    {
      ...
      if (K2 < 0.0) dolfin::error("Strange stuff: K2 < 0.0");
      ...
      if (ds < -10.0 * DOLFIN_EPS)
      {
        ...
        std::cout << "sb:" << sb << "\n";
        std::cout << "ds:" << ds << "\n";
        dolfin::error("Strange stuff: ds < 0.0");
      }
      ...
      for (std::size_t j = 0; j < n; j++)
      {
        ...
        if (L < 0.0 && _rotation) dolfin::error("Strange stuff: L < 0.0");
        if (E < 0.0) dolfin::error("Strange stuff: E < 0.0");

        // Evaluate ansatz
        const double f = ansatz(E, L);
        if (f < 0.0) dolfin::error("Strange stuff: f < 0.0");
  ...
  // Set fields
  void set_fields(std::shared_ptr<const dolfin::Function> NU,
                  std::shared_ptr<const dolfin::Function> BB,
                  std::shared_ptr<const dolfin::Function> MU,
                  std::shared_ptr<const dolfin::Function> WW)
  {
    _NU = NU;
    _BB = BB;
    _MU = MU;
  ...
  // The parameters
  dolfin::Parameters ev_parameters;

  // Member functions (to be defined by specific ansatz)
  //%(member_functions)s

  // Ansatz function to be specified by subclasses. 
...
protected:
  ...
  // The fields
  std::shared_ptr<const dolfin::Function> _NU;
  std::shared_ptr<const dolfin::Function> _BB;
  std::shared_ptr<const dolfin::Function> _MU;
  std::shared_ptr<const dolfin::Function> _WW;
